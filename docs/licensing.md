---
title: "Licensing"
description: "AGPL-3.0-or-later licensing terms for LoupePDF — what hosts can do, what they must do, and how to request alternative terms for proprietary use."
group: "Project"
order: 11
---
...
LoupePDF is published under **AGPL-3.0-or-later** — the GNU Affero
General Public License, version 3 or any later version. The full
licence text lives in [`LICENSE`](https://github.com/Printwithsynergy/loupe-pdf/blob/main/LICENSE)
...
If you distribute LoupePDF (binary or source) — or **make it available
...
- License any larger work that links against LoupePDF under
  AGPL-3.0-or-later.

The "**make available over a network**" clause (§13 of AGPL-3) is the
key difference from GPLv3: a SaaS that ships LoupePDF — even if no one
ever downloads a binary — has to offer the source to its users. Hosting
a hosted PDF viewer that imports `@printwithsynergy/loupe-pdf` triggers
this.
...
LoupePDF you also ship those dependencies, so check their notices in
...
proprietary or closed-source products. If you want to embed LoupePDF
...
LoupePDF is the rendering core for the printwithsynergy OSS PDF
